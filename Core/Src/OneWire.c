...
/**
    ...
    @date   11.07.2016
    @brief      @brief Implementación del protocolo 1wire basado en la biblioteca libopencm3 para el microcontrolador STM32F103
 	 	 	 	 	 	Quizás la biblioteca funcione correctamente en otros Reino Unido (se requiere verificación).
 	 	 	 	 	 	Es necesario realizar una verificación para garantizar que la configuración de UART/USART sea correcta para el funcionamiento.
 	 	 	 	 	 	en modo semidúplex
 	 	 	 	 	 	La idea general es utilizar hardware USART uK para simular el funcionamiento con 1 cable.
 	 	 	 	 	 	Los dispositivos se conectan al USART seleccionado al pin TX, que debe estar conectado a la línea
 	 	 	 	 	 	de alimentación con una resistencia de 4,7K.
 	 	 	 	 	 	La implementación de la biblioteca corta RX a TX dentro del Reino Unido, dejando el pin RX disponible para su uso en otras tareas.
 	 	 	 	 	 	La implementación de la biblioteca supone un posible trabajo simultáneo con ambos buses independientes y todos a la vez.
 	 	 	 	 	 	posible UART/USART en el microcontrolador. En este caso, todos los autobuses (hasta 5 unidades) serán abordados y
...
#define MAXDEVICES_ON_THE_BUS 1  // cambio por uno
...
void owReadHandler() { //controlador de interrupciones USART
	uint8_t index = getUsartIndex();
	/*Comprueba que hemos provocado una interrupción debido a RXNE. */
	if (((OW_USART->CR1 & USART_CR1_RXNEIE) != 0) &&
			((OW_USART->SR & UART_FLAG_RXNE) != (uint16_t)RESET)) {

		/* Recibir datos de la periferia y restablecer la bandera*/
		while ((OW_USART->SR & UART_FLAG_RXNE) == (uint16_t)RESET){;}
		rc_buffer[index] = USART_ReceiveData(OW_USART);
		recvFlag &= ~(1 << index);//restablecer la respuesta del indicador recibida después
	}
}
...
int get_ROMid (void){
	if (owResetCmd() != ONEWIRE_NOBODY) {    // is anybody on the bus?
		devices = owSearchCmd(&ow);        // obtener el ROMid de todos los dispositivos en el bus o devolver un código de error
		if (devices <= 0) {
			while (1){
				pDelay = 1000000;
		...
		i = 0;
		for (; i < devices; i++) {//enviar a la consola todas las ROM encontradas
			RomCode *r = &ow.ids[i];
...
void get_Temperature (void)
{
	...
	for (; i < devices; i++) {
		switch ((ow.ids[i]).family) {// cual es el sensor
		case DS18B20:
			// se devuelve el valor de la medicion anterior!
			t = readTemperature(&ow, &ow.ids[i], 1);
			Temp[i] = (float)(t.inCelsus*10+t.frac)/10.0;
			Tempi[0]=  (int8_t)(t.inCelsus);
			Tempi[1]= (int8_t)t.frac*10;
			break;
		/*case DS18S20:
			t = readTemperature(&ow, &ow.ids[i], 1);
			Temp[i] = (float)(t.inCelsus*10+t.frac)/10.0;
